...
log.log(repet,u"Librería 'exportar_mapas' cargada con éxito")
...
def quitar_acentos(texto):
    ...
    log.log(repet,u"'quitar_acentos' iniciando para {} de tipo {}...".format(texto,type(texto)))
    ...
    try:
        ...
        if texto == cadena_sin_acentos:
            ...
            log.log(repet,u"se ha aplicado el proceso 'quitar_acentos' para '{}' cambiado a '{}'".format(texto, cadena_sin_acentos))
    except Exception as e:
        log.log(repet,u">> ERROR. Se ha producido un error en el proceso de sustitución de acentos")
        log.log(repet,str(e))

    tiempo_quita_acentos_fin = datetime.datetime.now().strftime(u"%Y-%m-%d %H:%M:%S")
    log.log(repet,u"tiempo total de librería 'quitar_acentos': {}".format(tiempo.tiempo([tiempo_quita_acentos_ini,tiempo_quita_acentos_fin])))

    log.log(repet,u"'quitar_acentos' finalizado para {}...".format(texto))
...
def exportar(r_dest, nombarch):
    ...
    try:
        ...
        ruta = ruta + ".pdf"
        log.log(repet,u"Archivo codificado: {}".format(ruta))
        ...
        log.log(repet,u"archivo {}.pdf exportado con éxito".format(arcpy.env.sinacentos))

    except Exception as e:
        log.log(repet,u">> ERROR. Se ha producido un error en el proceso de exportación del archivo '{}'".format(ruta))
        log.log(repet,str(e))

    tiempo_exportar_fin = datetime.datetime.now().strftime(u"%Y-%m-%d %H:%M:%S")
    log.log(repet,u"tiempo total de librería 'exportar': {}".format(tiempo.tiempo([tiempo_exportar_ini,tiempo_exportar_fin])))    
    
    log.log(repet,u"'exportar_mapas' para '{}' finalizado!".format(ruta))
