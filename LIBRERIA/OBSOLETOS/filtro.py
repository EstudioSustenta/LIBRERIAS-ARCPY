...
log.log(repet,u"Librería 'filtro.py' cargado con éxito")
...
def fil_expr(capa_nombre, expr):     #Esta función acepta la cadena de expresión completa, ejemplo:---> "NOM_MUN = 'Jesús María' AND NOM_ENT = 'Aguascalientes'"
    ...
    log.log(repet,u"'filtro.fil_expr' iniciando para " + capa_nombre + u" con la expresión " + expr)
    
    try:
        capa = arcpy.mapping.ListLayers(mxd, capa_nombre)[0]
        capa.definitionQuery = expr
        log.log(repet,u"Filtro {} aplicado correctamente para la capa {}".format(expr, capa_nombre))
    except Exception as e:
        log.log(repet,u">> ERROR, el proceso filtro '{}' falló para la capa '{}'".format(expr, capa_nombre))
        log.log(repet,str(e))

    log.log(repet,u"Se aplicó filtro con la expresión {} para la capa {}".format(expr, capa_nombre))

    tiempo_fil_expr_fin = datetime.datetime.now().strftime(u"%Y-%m-%d %H:%M:%S")
    log.log(repet,u"tiempo total de librería 'fil_expr': {}".format(tiempo.tiempo([tiempo_fil_expr_ini,tiempo_fil_expr_fin])))

    log.log(repet,u"'filtro.fil_expr' finalizado para " + capa_nombre + u" con la expresión " + expr)
...
def aplicar_defq(capa_nombre, campo, texto_a_buscar):
    ...
    log.log(repet,u"'filtro.plicar_defq' iniciando para " + capa_nombre + u" con la expresión " + campo)
    ...
    try:
        ...
        log.log(repet,u"Definition query aplicado exitosamente.")
        
    except Exception as e:
        log.log(repet,u">> ERROR, el proceso aplicar_defq falló")
        log.log(repet,str(e))

    tiempo_aplicar_defq_fin = datetime.datetime.now().strftime(u"%Y-%m-%d %H:%M:%S")
    log.log(repet,u"tiempo total de librería 'aplicar_defq': {}".format(tiempo.tiempo([tiempo_aplicar_defq_ini,tiempo_aplicar_defq_fin])))

    log.log(repet,u"'filtro.plicar_defq' finalizado para " + capa_nombre)
...
def deshacer_defq(capa_nombre):
    ...
    try:
        ...
        log.log(repet,u"Definition query eliminado, mostrando todos los objetos.")
        
    except Exception as e:
        log.log(repet,u">> ERROR. Se ha producido un error aplicando 'filtro.plicar_defq' para: " + capa_nombre)
        log.log(repet,str(e))
    
    tiempo_deshacer_defq_fin = datetime.datetime.now().strftime(u"%Y-%m-%d %H:%M:%S")
    log.log(repet,u"tiempo total de librería 'deshacer_defq': {}".format(tiempo.tiempo([tiempo_deshacer_defq_ini,tiempo_deshacer_defq_fin])))
