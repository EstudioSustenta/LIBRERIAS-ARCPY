...
def shp(ruta_base_datos, ruta_shapefile):
    ...
    tabla_marginacion = 'marginacion'
    tabla_salud = 'salud'
    tabla_poblacion = 'poblacion'
    campo_clave= 'CVEGEO'

    # Conexión a la base de datos SQLite
    engine = create_engine(f'sqlite:///{ruta_base_datos}')

    consulta_sql_marginacion = f'SELECT * FROM {tabla_marginacion}'     # Consulta SQL para seleccionar los datos de la tabla 'migracion'
    ...
    # Convertir la columna 'CVEGEO' a tipo de datos 'str'
    df_marginacion['CVEGEO'] = df_marginacion['CVEGEO'].astype(str)
    df_salud['CVEGEO'] = df_salud['CVEGEO'].astype(str)
    df_poblacion['CVEGEO'] = df_poblacion['CVEGEO'].astype(str)
    ...
    return(f'Se ha completado la anexión de columnas de población y marginación al archivo "{ruta_shapefile}"')
